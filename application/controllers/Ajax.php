...
class Ajax extends CI_Controller
{
    ...
    public function inputabsen()
    {
        ...
        if ($this->M_siswa->CekSiswa($nis) < 1) {
            ...
            // cek jam absen
            $jam_masuk = $this->db->query("SELECT * FROM tb_jam_absen WHERE type = 'Masuk' ")->result_array()[0]['mulai'];
            $jam_selesai_masuk = $this->db->query("SELECT * FROM tb_jam_absen WHERE type = 'Masuk' ")->result_array()[0]['selesai'];
            $jam_keluar = $this->db->query("SELECT * FROM tb_jam_absen WHERE type = 'Keluar' ")->result_array()[0]['mulai'];
            $jam_selesai_keluar = $this->db->query("SELECT * FROM tb_jam_absen WHERE type = 'Keluar' ")->result_array()[0]['selesai'];
            $jam_telat_masuk = $this->db->query("SELECT * FROM tb_jam_absen WHERE type = 'Terlambat' ")->result_array()[0]['mulai'];
            ...
            $tglskrg = date('Y-m-d');
            $cekabsen = $this->db->query("SELECT * FROM tb_detail_absen WHERE nis = '$nis' AND masuk = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
            if ($type == 'masuk') {
                if ($cekabsen > 0) {
                ...
                $cekabsenkeluar =     $cekabsen = $this->db->query("SELECT * FROM tb_detail_absen WHERE nis = '$nis' AND masuk = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
                if ($cekabsen == 0) {
                    ...
                    $waktu = time();
                    $tanggalFormat = date("Y-m-d H:i:s", $waktu);
                    $this->db->query("UPDATE tb_detail_absen SET keluar = '1', jam_pulang = '$tanggalFormat' WHERE nis ='$nis' AND tanggal_absen = '$tanggal' AND masuk = '1'");
                    $datasiswa = $this->M_siswa->detailsiswa($nis);
                    $datasiswa2 = $datasiswa[0];
    ...
    public function linked_nis()
    {
        ...
        $ceknis = $this->db->query("SELECT * FROM tb_siswa WHERE nis = '$nis'");
...
';
    ...
    public function login_proccess()
    {
        ...
        $biosiswa = $this->db->get_where('tb_siswa', ['nis' => $nis])->result_array()[0];
    ...
    public function tampilizin()
    {
        ...
        $dataizin = $this->db->query("SELECT * FROM tb_izin JOIN tb_siswa on tb_siswa.nis = tb_izin.nis_siswa WHERE id = '$idizin' ")->result_array();
    ...
    public function konfirmasi_izin()
    {
        ...
        $dataizin = $this->db->query("SELECT * FROM tb_izin JOIN tb_siswa on tb_siswa.nis = tb_izin.nis_siswa WHERE id = '$idizin' ")->result_array()[0];
        ...
        if ($aksi == 'Diterima') {
            ...
            $this->db->query("INSERT INTO tb_detail_absen VALUES ('','$jam','$tanggal','$nis','$keterangan','$kodekelas','$kodejurusan','1','1')");
            $this->db->query("UPDATE tb_izin SET status = 'Diterima'  WHERE id = '$idizin'");
            $this->session->set_flashdata('flash', ['alert' => 'success', 'message' => 'Izin berhasil di konfirmasi, terima']);
        } else if ($aksi == 'Ditolak') {
            $this->db->query("UPDATE tb_izin SET status = 'Ditolak' WHERE id = '$idizin'");
    ...
    {
        $email = $this->input->post('email');
        $queryemail = $this->db->query("SELECT * FROM tb_user  WHERE email = '$email'");
        if ($queryemail->num_rows() < 1) {
            ...
            $this->db->query("UPDATE tb_user SET kode_unik = '$kodeacak' WHERE email = '$email' ");
    ...
    {
        $nis = $this->input->post('nis');
        $querynis = $this->db->query("SELECT * FROM login_siswa JOIN tb_siswa on tb_siswa.nis = login_siswa.nis_siswa WHERE nis_siswa = '$nis'");
