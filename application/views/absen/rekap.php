<div class="content-body">
    <!-- row -->
    <?php
    $sqlsabtu = "SELECT status FROM tb_libur WHERE type = 'weekend' AND keterangan = 'sabtu' ";
    $sabtu = $this->db->query($sqlsabtu)->result_array()[0]['status'];
    $sqlminggu = "SELECT status FROM tb_libur WHERE type = 'weekend' AND keterangan = 'minggu' ";
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        $bulan = $_GET['bulan'];
                        ?>
                        <a onclick="htmlTableToExcel('xlsx')" class="btn btn-primary"> <i class="fas fa-file-export"></i> Download rekapan</a>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            ...
                            <form action="<?= base_url() ?>absen/input" method="POST">
                                <table class="table table-bordered table-striped with-check" id="tblToExcl">
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <?php for ($tanggal_table = 1; $tanggal_table <= 31; $tanggal_table++) {
                                                echo "<th colspan='2' class='text-center'>$tanggal_table</th>";
                                            } ?>
                                        ...
                                        <?php foreach ($siswa as $d) :  ?>
                                            ...
                                            <tr>
                                                ...
                                                if (isset($_GET['bulan']) && isset($_GET['kelas'])) {
                                                    $bulan = $_GET['bulan'];
                                                    $kelas = $_GET['kelas'];
                                                    $sql = "SELECT * FROM tb_detail_absen WHERE nis = '$nis' AND kode_kelas = '$kelas' AND tanggal_absen LIKE '%$tahun-$bulan%' ORDER BY tanggal_absen ASC";
                                                } else {
                                                    $sql = "SELECT * FROM tb_detail_absen WHERE nis = '$nis' AND  kode_kelas = '$kelass' AND tanggal_absen LIKE '%$tahun-$cari_bulan_siswa%' ORDER BY tanggal_absen ASC";
                                                    $bulan = date('m');
                                                }
                                                ...
                                                if ($query->num_rows() > 0) {
                                                    ...
                                                    foreach ($query->result_array() as $absen) {
                                                        $waktuMasuk = new DateTime($absen['jam_absen']);
                                                        $waktuPulang = new DateTime($absen['jam_pulang']);
                                                        //mengabil tanggal 
                                                        ...
                                                        for ($nomor = $nomor2; $nomor <= $ambil_tanggal[2]; $nomor++) {
                                                            ...
                                                            // cek libur
                                                            $sqllibur = "SELECT * FROM tb_libur WHERE tanggal = '$tahun-$bulan-$tgl' AND status = 'Aktif'";
                                                                ...
                                                                if (count($datalibur) > 0) {
                                                                    ...
                                                                    if ($absen['keterangan'] == 'h') {
                                                                        if ($absen['masuk'] == 1 && $absen['keluar'] == 1) {
                                                                            echo '<td>' . $waktuMasuk->format("H:i:s") . '</td>';
                                                                            echo '<td>' . $waktuPulang->format("H:i:s") . '</td>';
                                                                        } else {
                                                                            echo '<td>' . $waktuMasuk->format("H:i:s") . '</td>';
                                                                            echo "<td></td>";
                                                                        }
